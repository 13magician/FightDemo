...
public class GreenWater : Monster {//绿水灵的脚本。我想在动画里设置跳起时候的播放速度0.接触地面才改回正常
   ...
   public float jumpForceX = 25f, jumpForceY=75f;//怪物跳起来的力
    ...
    public string deathAnimName = "death_greenWater";
    public AudioClip deathAudio,wasAttackedAudio;//死亡和被攻击音效
    void Start () {//在基类重定义吧··
    ...
    public override void WasAttacked(float wasAttackedDurationTime, Transform attacker)//被攻击接口？
    {
        wasAttackedEndTime = wasAttackedDurationTime;//设置被攻击持续时间
        AudioSource.PlayClipAtPoint(wasAttackedAudio, transform.position);//播放被攻击音效
        HPduration = 10f;//设置血条持续时间
        if(currentHP<= 0f)
        {
            attacker.GetComponent<PlayerControl>().killGreenWaterNum++;//杀死绿水灵次数加1
        }
    }
    void Update()
    {
    ...
    {
        if (hit.transform.tag == "Player" &&hit.gameObject.GetComponent<PlayerControl>() != null)//碰到的是玩家··不用那么多判断
        {
            Transform hitTf = hit.transform;
            ActionState playState = hitTf.GetComponent<ActionState>();//获取玩家状态
            if (playState.isDeath) return;//如果玩家死亡直接返回
            if (playState.unmatchedTime == 0.0f)//条件是没有无敌持续时间
            {
                playState.unmatchedTime = 1.1f;//给玩家添加无敌时间
                hitTf.GetComponent<PlayerControl>().anim.Play("wasAttacked");//播放被攻击动画
                hitTf.GetComponent<PlayerControl>().currentHP -= Random.Range(2, 5);//减少玩家血量，2-4点
    ...
    void wasAttackedAnim()//被攻击处理动画
    {
        ...
        if (wasAttackedEndTime <= 0 &&isGround)//如果被攻击动画结束时间小于0，并且是在地面
        {
    ...
    void FixedUpdate()
    {
        if (currentHP <= 0 && !IsName(deathAnimName))//不处于死亡动画中
        {
            AudioSource.PlayClipAtPoint(deathAudio, transform.position);//播放死亡声音
